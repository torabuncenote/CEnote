...
- **ヘルプ（他部署からの応援）は `base:'help'`。** 勤務表の「サポート」行（見出しが `SUPPORT_ROW_RE` に一致する行）は日付セルに勤務コードではなく人名が書かれているので、`parseShiftSheet` がそれを `D.shift[ym][名前][日]={shift:'ヘルプ', help:true}` として取り込み、`getShiftForDay().help` → `plDerive` の順で名簿に載る。**`D.stf` には登録しない**（分院のスタッフではないため）。1セル複数名は「、・,／改行」で区切り、**空白では割らない**（「田中 太郎」を2人に割ってしまう）。手入力は配置盤の「＋追加」モーダル下部の欄（`addHelpStaffFromModal`）から。ここは職員名を入れる欄なので**氏名パターンの黄色警告では止めない**（メーカー担当者名と同じ判断。`phiHasBlock` の対象＝患者IDなどだけ弾く）。`validateShiftData` の「名簿に未登録」警告もヘルプには出さない。
