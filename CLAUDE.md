...
- **ヘルプ（他部署からの応援）は `base:'help'`。** 勤務表の下部にある「サポート」欄は**2行1組**——見出し行（col2 が `SUPPORT_ROW_RE` に一致）の日付セルに**名前**、そのすぐ下の行の同じ日付セルに**役割**（HDのコード）が入る（スタッフ行が「職員番号行→氏名行」の2行組なのと同じ）。実データではこの組が2つ並ぶ。`parseShiftSheet` がこれを読み `D.shift[ym][名前][日]={shift:役割, help:true}` として取り込み、`getShiftForDay().help` → `plDerive` の順で名簿に載る（役割があればHD側のその持ち場、無ければCEの未割当）。役割は `plHdRole()` を通すので「休」等は役割にならない。**`getShiftForDay` ではヘルプを CE/OC/HD のどの枠にも入れないこと** — HDの役割コードを持つので、素通しすると通常のHD勤務者として二重に数えられ、HD担当表にも分院スタッフとして並ぶ。**`D.stf` には登録しない**（分院のスタッフではないため）。1セル複数名は「、・,／改行」で区切り、**空白では割らない**（「田中 太郎」を2人に割ってしまう）。手入力は配置盤の「＋追加」モーダル下部の欄（`addHelpStaffFromModal`）から。ここは職員名を入れる欄なので**氏名パターンの黄色警告では止めない**（メーカー担当者名と同じ判断。`phiHasBlock` の対象＝患者IDなどだけ弾く）。`validateShiftData` の「名簿に未登録」警告もヘルプには出さない。
