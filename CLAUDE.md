...
- CE日ページ側（`variant='ce'`）は `.staffz` 折りたたみシェルを流用し、対象0件でも**編集中なら見出しごと出す**（全員を控えへ移した瞬間にセクションが消えて編集に戻れなくなるのを防ぐため）。**折りたたみを閉じたまま「✏️ 役割を編集」を押しても中身が見えるよう、`hdRosterToggleEdit` は非編集→編集に入る瞬間だけ `_hdRefOpen` を強制的に開く**（閉じたままだと `#hd-ref-body` が `display:none` で編集モードに入っても何も見えないため）。
