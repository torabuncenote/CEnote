...
**`opsItemSup(it)` は1要素の中の「、」「，」「,」も分割する。** 旧・自由記述欄はカンマ(`,`)でしか分けていなかったため、読点で打たれた「5mm、VIO、ウロタワー」が1品として保存されていた（実データで153症例中49症例）。**保存データは書き換えず読むときだけ分ける**ので、チップ・集計・CSV・絞り込みは1品ずつになり、その症例を編集すると分けた形で保存し直される。一括の書き換え（移行処理）にしなかったのは、145日分の連絡表を一度に書き換えると戻すのにバックアップ復元が要るため。`item.sup` を読むときは必ず `opsItemSup` を通すこと——生で読むとこの分割が効かず、同じ物品が集計で分かれる。
