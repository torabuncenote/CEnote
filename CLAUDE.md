...
**操作ログのCE/HD主観**: `writeLog()` writes the current `_viewMode` into each entry as `view` (`'ce'|'hd'`) — the ~80 existing call sites need no changes, it's picked up automatically. Logs written before this field existed have no `view`; reading always goes through `logViewOf(l)` (defaults to `'ce'`, mirrors the board's `boardKindOf`). Admin actions (master-data edits, user permissions, backups, etc.) are classified by whatever subjective mode was open at the time, so the ログ screen's default tab is **「すべて」**, not `'ce'`/`_viewMode`-linked — otherwise an admin watching only one tab would miss the other's admin actions. Display filters by subjective mode first, then caps at `LOG_SHOW_MAX` (100); fetch size is fixed at `LOG_FETCH_MAX` (3000 — ~60 days' worth at the observed ~35 logs/day). There used to be a「⬇️ さらに読み込む」button that grew the fetch size, but since the display cap never moved, pressing it never showed more rows — removed as confusing.

**ログの自動削除**: `cleanOldLogs()` deletes logs older than `logKeepDays()` (default 60 — return value of `D.autoDelCfg.logKeepDays`, falling back to `LOG_KEEP_DAYS_DEF`; `0` disables it) in one login-time pass, capped at once per day via `D.autoDelCfg.logLastClean`. **This runs independently of `D.autoDelCfg.enabled`** (the 連絡表/画像 auto-delete toggle) — logs cost everyone's bandwidth if left to grow, whereas skipping them doesn't stall any workflow, so it must not be nested inside `checkAutoDelTiming()`'s `!enabled` early-return. **`D.autoDelCfg`'s default-value literal (`{enabled:false,period:365,interval:30,lastClean:0}`) is duplicated at several call sites — do not add `logKeepDays`/`logLastClean` to any of them.** `logKeepDays()` fills in the default at read time instead (same pattern as `ensureStaffZone`/`hdCountSpArr`), so a missed call site can never leak a stale value. Deletion is a keyed bulk `update({key:null,...})` in batches of 500 (`logSplitBatches(keys)`) rather than removing one key at a time — the first run had ~1,300 rows to clear and per-key removal would be far too slow.

**ストレージ画面が見せる数字**: `renderStorageUsage()` は長らく `/data` の内訳しか出しておらず、実測ではFirebaseコンソールの45MBに対して1.2MBしか表示していなかった。**乖離の正体はほぼ全部が `/backups`**（1個が `D` 全体のコピーで約0.6MB、実測75個で約45MB）。いまは `/data` に加えて `/board`・`/tasks`・`/taskMemos`（同期。クライアントのキャッシュから `calcBytes`）と、`/logs`・`/backups`（非同期で後埋め）を出し、`_updateStorageGrandTotal()` が総量を出してコンソールと突き合わせられるようにしてある。**`/backups` は丸ごと読むと数十MBを転送してダウンロード枠を食うので、「`/backup_meta` の個数 × 実体1個のサイズ」で概算する** — 全件取得に変えないこと。
...
| `refreshHdRoster(ds)` / `hdRoleOccupant(ds, role, except)` / `hdRosterMove(ds, name, spec)` / `openHdRolePicker(ds, name)` | HD担当表（`#hd-roster-box`）の再描画の唯一の入口／role の現担当者（1人のときだけ）／取り消し付きの移動／配置盤の「役割」枠に落としたときに出す役割選択ダイアログ |
...
| `treeEditOk()` | オペ/カテ術式マスタとHD特殊治療マスタの書き込み権限（`can('mst')` または `canEdu()`）。業務マスタと教育項目マスタの両方の入口がこれを通る |
...
| `maybeShowWhatsNew()` | 更新後に初めて開いたときだけ、変更点を1回ポップアップで知らせる（既読は端末ローカル `ce2_seenseq`）。詳細は Changelog System 節 |
...
- **旧フィールドは消さない。** `plDerive()` が読み出し時に吸収する（`ensureStaffZone` と同じ方針）。`staffZone.ce[名前]==='hd'` は「HDへ応援（受け取り済み）」、`staffZone.hd[名前]==='to_ce'` は「CEへ応援」として読む。`hdDuty.overrides` の `{元の人:差し替え後}` は、元の人を名簿から外して差し替え後の人にその役割を持たせる形に読み替える。**`plSpot()` の hdrole 判定は `spot` を必ず `plHdRole()` で正規化してから返す** ので、旧データや誤った値（かつての `spot:'応援'` を含む）が入っていても役割名としてそのまま画面に出ることはなく、単に「役割なし」として扱われる。
...
- **HD担当表（`buildHdRoster`/`hdShiftWorkers`）は台帳から作る。** `side==='hd'` かつ `plSpot().kind==='hdrole'` の人だけが並ぶ（余剰・ゾーンの人は配置盤側に出る）。役割は `spot || shift`。**`plMove` 側で「同じ役割の人を無条件に押し出す」ことはしない** — 準夜は3名固定のように同じ役割を複数人で持つ枠があり、押し出すと3名のうち1人を触っただけで残り2名が余剰へ落ち、担当表に1枠しか残らなくなる（実際に起きた）。
- **役割の入れ替えは `plMove` の `spec.with`（日勤だけ）で1対1に表現する。** `hdRoleOccupant(ds, role, except)` がその role にいま「ちょうど1人」いるときだけその氏名を返す（0人・2人以上の異常データ・準夜は `''`）。`plMove(ds, name, {kind:'hdrole', role, with})` は、相手が実際にいま `role` に居るときだけ相手も動かす——動かす人の元が hdrole で role を持っていれば相手はそこへ、それ以外（控え・余剰など）なら相手はHD余剰へ。相手の書き換えは `plMove` の中で直接行い（再帰呼び出しはしない）、`saveD`/`writeLog` は1回にまとめ、ログ詳細に相手の移動も含める。準夜は `with` を無視する（人数を増やすだけで入れ替えではないため）。
...
**HD担当表の役割編集（`#hd-roster-box` / `refreshHdRoster`）**：HD日ページの「🩸 HD担当表」とCE日ページの相互参照は、同じ入れ物・同じ関数群で描く1つの共通部品——CE/HDの双子関数を増やさない方針の延長で、以前あった読み取り専用の `hdRosterRefHTML`（CE日ページ用）は廃止し、**CE日ページからも編集できる**ようにした（同じ日の同じ現場なので、どちらの主観からでも動かせてよい、という判断）。
- 安定要素 `<div id="hd-roster-box" data-ds data-variant="hd"|"ce">` の中身は `refreshHdRoster(ds)` が作る（`hdRosterBoxHTML`→`hdRosterTableHTML`→通常表示 `hdRosterNormalTableHTML` / 編集表示 `hdRosterEditTableHTML`）。**`renderPlacementBoard(ds,...)` が早期returnより前で必ず `refreshHdRoster(ds)` を呼ぶ** ので、`plMove`/`plOffer`/`plCancelOffer`/`plAccept`/`plEditMemo`/名簿の増減/`plResync` のどの経路を通っても、同じ画面のHD担当表が自動で揃う——個々の呼び出し側が更新を意識する必要はない。`renderPageHD`/`renderPage` は入れ物を置いた直後にも1回 `refreshHdRoster(ds)` を呼ぶ（`renderPlacementBoard` までの間に空のまま描画される瞬間を作らないため。二重に呼んでも軽い）。
- **通常表示**は居る人の行だけ（役割なしは「—」）。**編集モード**（`✏️ 役割を編集`、`can('duty')` のときだけ、モジュール変数 `_hdRosterEdit`＝編集中の `ds`、非永続）は日勤 `HD_DAY_CODES` の8行を必ず出し（空きは点線）、準夜は居る人＋追加行、役割なし・HD余剰の人は下の「控え」枠にまとめる。操作は「名前をタップ→役割をタップ」（`_hdRosterPick` で選択状態を保持）と「長押しドラッグ」（配置盤 `bindPlacementDnD` と同じ250ms・8pxスロップの規則を `hdRosterBindEvents` 内に独立実装——配置盤のコード自体は共通化しない）の両方に対応。**別の日を開くと編集は自然に解除される**——`refreshHdRoster(ds)` が、いま開こうとしている `ds` が `_hdRosterEdit` と違えば冒頭でリセットする。
- 確定は必ず `hdRosterMove(ds,name,spec)` を通す（`plMove` を直接呼ばない）。日勤の役割に人が居れば `hdRoleOccupant(ds,role,except)` で1対1の入れ替え（`spec.with`）になり、取り消し付きトーストを出す。
...
**マイ担当（`renderMySchedule`）はHDの役割も出す**：CE担当枠（`getDutyCfg`＋`dat.duties`）とスケジュールブロックに加えて、`buildHdRoster(ds)` から自分の行のシフトコード（M・A1・準夜等）を `🩸 ` 付きで並べる。**`_viewMode` で分岐させない** — その人がその日HD勤務かどうかは主観と無関係な事実で、HD勤務が無い人は `getShiftForDay(ds).hd` に載らず何も出ない。`buildHdRoster` 経由なので台帳（`hdRosterMove`/`plMove`）での役割変更もそのまま反映される。

**CE⇄HD 相互参照**：同じ日の同じ現場を別主観で見ているだけなので、相手側の割り当ては見えるべき、という方針。`ceRosterRefHTML(ds,dat)` がHD日ページに「🩺 CE担当表」（`getDutyCfg`＋`dat.duties`、枠の色は`dutyColorFor(slot)`）を出す——こちらは今も**`<select>`を使わない読み取り専用のテキスト表示**で、編集はCE主観の担当カードで行う。折りたたみの外枠は人員配置管理と同じ`.staffz`シェルを流用（新規CSS不要・見た目も揃う）、開閉は`_ceRefOpen`（非永続、既定は閉じる）。対象が0件のときは見出しごと出さない。印刷CSSでは`#ce-ref-body`を`#staffz-body`と同様に強制表示する。CE日ページに出す「🩸 HD担当表」は**読み取り専用ではなくなった**——上記「HD担当表の役割編集」節の `#hd-roster-box`（`variant='ce'`）そのもので、開閉は同じく`_hdRefOpen`/`#hd-ref-arrow`/`#hd-ref-body`を使うが、中身の描画・編集イベントは `hdRosterRefHTML` ではなく `refreshHdRoster(ds)` が担う。
...
- 作りは使用物品の `openSupPickerModal` と同じ：開いた時点のマスタを複製して持ち、`onclick` には添字だけを渡す。科ごとの折りたたみ→中カテゴリの小見出し→術式ボタン（1つ選ぶ）、自由入力（科を選んでから名前）、入室時間（端末標準の `<input type="time" step="300">`＋「AMOC」「PMOC」の2ボタン。スマホは端末の時刻選択、PCは打ち込み・矢印キーで5分ずつ。**値は1分単位のまま保存し丸めない**——`step` はPCの矢印キーのためだけで、スマホの端末はこの指定を無視する。一度自前のリールにしたが、PCのマウスで回しにくく端末標準に戻した）、ロボットなら配置。入力中はポップアップ全体を描き直さず、`opsPickRefreshTime()` が上の欄・濃淡・AMOC/PMOCの選択表示だけを直す（全体を描き直すと入力中の端末のピッカーが閉じる）。触らなければ既存の値（7:07など）は変えない。入力欄の値は `opsTimeInputVal(h, m)` で2桁の `HH:MM` にそろえる（`9:05` のままでは入力欄が受け付けない）。
...
**入室時間ピッカー**: `withTime=true` の行は、カテのブリーフィング欄と同じ「○時：△分」方式（ツリー版は上記ポップアップの端末標準の時刻入力＋AMOC/PMOC、フラット版は2セレクト `makeTimeHourOpts`/`makeTimeMinuteOpts`）。○は8〜16時＋AM／PM、△は0〜55分（5分刻み）＋OC。値は `item.time` に単一文字列で保存（`combineItemTime(h,m)` で結合、`parseItemTime(t)` で復元。旧形式 `"8:15"` `"AMOC"` `"PMOC"` も読める）。「自由入力」ボタンで `item.time='__free__'` に切替えるとテキスト入力（`item.timeTxt`）に変わる。
...
- **移行前の端末（`_migVer` < 6）だけ `supMasterOf()` が旧マスタから作って見せる。** 移行後に種類を全部消したときに旧マスタが生き返らないよう、「空かどうか」ではなく `_migVer` で判定する（Firebaseは空配列を保存しないので区別できない）。**このフォールバックは `supTree` だけでなく、さらに古いフラットな `D.sup` も同じ `_migVer` で塞ぐこと** — `D.sup` は保存が無ければ `DEF_SUP` で埋め直されるので実質空にならず、`supTree` 側だけ塞いでも6品目が「その他」として生き返り、次にマスタを編集した時点で `supMasterForWrite` が `D.supMaster` に焼き付けてしまう
...
**本番環境では画像は実質すべて Storage ではなく RTDB（`/data` 等）へ base64 で直接入っている。** 実データで測定したところ Storage 実測は常に0件だった——本プロジェクトは Spark（無料）プランのままで Cloud Storage for Firebase が使えず、`uploadToStorage` が毎回失敗して `_uploadFallback` の base64 経路（`media.url` または `.data` に `'data:image/...'` を直接保存）へ黙って切り替わっているため。この構成は現状のまま運用する判断をしている（無料枠1GBの範囲で足りる）。そのため `renderStorageUsage` は RTDB 側の画像容量が主役——`countEmbeddedMedia()` が `D.pages[*].memos`/`.hdMemos`（本文＋返信）・`D.manual`・`_boardData`（投稿＋返信）・`_tasksData`・`_memosData` を横断し、`url`/`data` が `'data:'` で始まるレコードだけを「DB埋め込み」として数える（`storagePath` を持つ＝実際にStorage経由のレコードは除外）。バイト数は文字列長そのもの（RTDBの実消費量であり、base64デコード後サイズには換算しない）。`measureStorageUsage()` の結果が0件のときは「合計 0B」ではなくこの実態（DB埋め込みの枚数・容量）を表示する。DB全体の容量バーも `/data` 単体ではなく `/data`+`/board`+`/tasks`+`/taskMemos` の合算に対して出す——別パスを合算しないと掲示板・タスクの画像がバーから丸ごと漏れる。
...
- HD側が `buildHdRoster` ではなく **`hdShiftWorkers(ds)` を直接使う**のは、並べ替えや行の組み立てが要らないため。どちらも同じ関数（＝台帳）を通るので、日ページのHD担当表で役割を変えた分は自動で反映される。
...
**到達度・実績は「OPE／カテ／病棟外回り／機器管理／透析／特殊治療」の6分類に統一している。** 担当枠やHD役割（M・A1・…・B・準夜）は「その日どこに立ったか」であってスキルではないので評価単位にしない——枠ごとの回数は CE公平性/HD公平性タブが既に担当しているため重複させない。フリー・遅出フリーは実績にだけ「その他」として日数を出す（到達度は持たない。フリー中心の人の実績が全項目0日になって「何もしていない」ように見えるのを防ぐため）。
...
- **教育項目マスタ画面は教育到達度6分類ぶんをまとめて編集できる。** 病棟外回り/機器管理/透析の細目（`D.eduItems`。編集権限は従来どおり `mst` ではなく `isAdmin || canEdu()`＝`eduItemMasterOk()`。`mst` を渡すと OPE/カテ術式マスタや担当枠まで触れてしまうので、余分な権限を渡さずに済ませる）に加えて、**オペ・カテの術式マスタ（`D.opeTree`/`D.cathTree`）とHD特殊治療マスタ（`D.hdTreatments`）もそのまま同じ画面に描く**——教育用の別リストは作らず、業務マスタ側の既存セクションと完全に同じデータを兼ねる（どちらから直しても同じ）ので、業務マスタ側のセクションは削除していない。この3つ（オペ・カテ・特殊治療）の書き込み系は `treeEditOk()`（`can('mst') || eduItemMasterOk()`）で判定する——業務マスタ（`mst`）からは従来どおり、教育到達度の権限だけを持つ人も同じ画面から編集できるようにするため、`D.eduItems` の権限とは別関数にしてある。**HD主観（`_viewMode==='hd'`）ではオペ・カテのブロック（`#edu-ope-block`/`#edu-cath-block`）を隠す**（HDに担当枠・症例の概念が無いため）。特殊治療は両主観で表示する。UIは `renderHdTreatList`（HD特殊治療マスタ。業務マスタ側・教育項目マスタ側の要素id〈`'hd-treat-list'`/`'edu-hd-treat-list'`〉のうちDOMにある方だけ描く）と同型の `renderEduItemList(kind)`/`addEduItem(kind)`/`rmEduItem(kind,i)`/`mvEduItem(kind,i,d)`（`kind` は `'ward'|'device'|'hd'`）。オペ・カテの術式ツリーは業務マスタ側の `renderTree`/`editTreeItem` をそのまま再利用し、`renderTreeAll(saveKey)` が `TREE_MOUNTS[saveKey]`（マウント先elIdの一覧）のうちDOMに存在する方だけ描き直す——`renderTree` 内の自己再描画（追加・削除・改名後の再描画）はすべてこれを通るので、業務マスタ側だけ開いていても教育項目マスタ側だけ開いていても両方が揃う。マスタタブの「🎓 教育項目マスタ」セクションは `data-perm` を使わない（pane-master Section Gating の対象外）——`renderEduItemMaster()` が `#edu-items-section`／`#edu-ope-block`／`#edu-cath-block` の表示切替を自前で行う。置き場所は🎓教育グループの `.mgrp-body` 直下で、そこは他セクションと同じ（マスタタブのグループ節を参照）。
...
- Completed tasks are kept but **folded**: `taskDoneSectionHTML(sec, pairs, isMemo, today, forceOpen)` renders a single `✅ 完了（N件）▸` header (open state in the non-persistent `_taskDoneOpen[sec]`, `sec` = `'all'|'person'|'memo'`, default closed; forced open when the 完了 filter chip is on), and when opened each item is a one-line `taskDoneRowHTML` (✅ tag = revert to 未着手, title, done date). Clicking a title expands just that item to its full card (`_taskDoneItemOpen['t:'|'m:'+id]`, collapsed again by the card's ▴ button). Private memos fold the same way. The staff load graph shows only 未着手/進行中 bars (scaled by the larger of those two) — 完了 is a count under the name (`✅ 完了 N`), since the graph compares what each person is currently carrying and a done bar only shrank the others.
- **Attachments on done tasks/memos are deleted `TASK_MEDIA_KEEP_DAYS` (7) days after `doneAt`** by `sweepDoneTaskMedia()`, called from the `/tasks` and `/taskMemos` listeners and on `swTab('task')` — **never from `renderTasks`** (rendering must not write). Deliberately not at the moment of completion: `/tasks`/`/taskMemos` are outside every backup, so one mis-tap on the status tag would destroy the images irrecoverably; reverting within 7 days resets `doneAt` and keeps them. Cards show the deletion date (`_taskMediaExpireNote`). The `/tasks` rule lets any authenticated user update, so whichever client sweeps first wins (idempotent); memos are only swept on the owner's client.
...
- **Update notice (`maybeShowWhatsNew()`)**: the first time the app is opened after an update, a one-time popup (dynamic `.ov`/`.md`, `#whatsnew-ov`, built the same way as `plDialog`) shows what changed since the device last saw it. The seen release number lives in `localStorage` (`ce2_seenseq`, a plain number string) — device-local like `ce2_theme`/`ce2_viewmode`, not part of `D`, never cleared on logout; all reads/writes go through `try/catch` (`whatsNewMarkSeen()`) so a blocked/full localStorage just means the popup never appears rather than throwing.
  - **A device with no key yet is silently marked as seen at the current `APP_SEQ` and shows nothing** — otherwise a brand-new device (or one that just cleared storage) would see every past release at once instead of only what's new going forward.
  - `whatsNewReleases(seenSeq)` only considers releases whose `ver` matches the new `Ver.YYYY/MM/DD-N` format (old-style `'2026.06y'` entries can never be "new"), filtered to those numbered above `seenSeq`, then filtered per-item like the two existing changelog views: general users go through the existing `filterChangelogForGeneral()` (drops `admin:true` and `t:'fix'`); admins keep `admin:true` items but still drop `t:'fix'` — a bug fix has no "here's what changed for you" framing for either audience, so it's excluded for both, not just general users. **On top of that, the popup is opt-in: only items carrying `notify:true` survive** (for both audiences) — `new`/`imp` alone is not enough. `notify:true` is added to a changelog item only after the author has confirmed that specific item one-by-one with the user in a table before merging; it is never set by default when writing a release entry. **The changelog screens (`renderChangelog`/`renderChangelogPublic`/`buildChangelogHTML`/`filterChangelogForGeneral`) are unaffected by `notify` and always show every item regardless** — only the popup is opt-in. Releases left with zero items after filtering are dropped; if filtering leaves nothing to show at all, it marks itself seen immediately rather than popping up empty.
  - The `<li>` markup for each item (`changelogTagHTML(it)` + `escH(it.d)`) is factored out of `buildChangelogHTML` so the popup reuses the exact same `cl-tag`/`cl-tag-new`/`cl-tag-imp` styling instead of duplicating the tag-label maps — `renderChangelog()`/`renderChangelogPublic()`'s output is unchanged by this.
  - **Marking as seen happens only when the popup is closed** (background click, 閉じる, or Escape all call `closeWhatsNew()`) — never on open — so reloading before closing shows it again.
  - Called from two places: the `/data` listener's first load, `setTimeout(maybeShowWhatsNew, 800)` placed right after `hideSyncOverlay(true)` so it doesn't collide with the "✅ 同期しました" toast; and preview-mode startup (`isPreview` branch in `init()`), so it can be exercised without Firebase.
  - Skipped entirely while `_staleApp` is true (an outdated open tab has no business announcing releases it doesn't actually run).
